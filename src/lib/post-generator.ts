const HASHTAGS = `#dieppediscdemons #dieppediscgolf #discgolfleague #discgolfdieppe #discgolfawesome #discgolflife #discgolfing #udisc #cityofdieppe #villededieppe #CTP`;
...
export function generateChampionshipPost(summary: ChampionshipSummary): string {
  ...
  return `🏆 Championship Night Recap – DDGMSL 🏆
Last night marked the conclusion of the Dieppe Disc Golf Mixed Summer League. ${summary.totalPlayers} players came out for an unforgettable finale.
...
${ctpLines ? `CTP Winners\n${ctpLines}\n\n` : ""}${aceLines ? `Ace Winners\n${aceLines}\n\n` : ""}Thank you to every volunteer and every player who made this season a success. Stay tuned for what's coming next!
...
export function generateNewspaperBody(summary: RoundSummary): string {
  ...
  return `DIEPPE, N.B. — Week ${summary.weekNumber} of the DDGMSL brought out ${summary.totalPlayers} players ready to compete.
...
Thank you to every player and volunteer who made the DDGMSL Championship Series possible.`;
}
